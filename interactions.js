...
import { YELLOW } from "./initialization";
import { isTerminalCommand, executeTerminalCommand, getCurrentWorkingDirectory } from './terminal-commands';
/**
...
function getClientForProvider(provider) {
    switch (provider) {
        ...
        case "openrouter":
            ...
            if (!openrouter)
                throw new Error("OpenRouter client not available or not configured. Make sure you have the @openrouter/ai-sdk-provider package installed and OPENROUTER_API_KEY set.");
            // OpenRouter supports both chat and completion models, but for simplicity, use .chat for now
            return (modelName) => openrouter.chat(modelName);
        default:
 ...
 */
export function runInteractiveSession(config, loadedTools, sessionFile, agentName) {
    const BLUE = "\x1b[34m";
    const RESET = "\x1b[0m";
    // Determine which prompt to use
    let promptString = '';
    // Check if we're running in shell-prompt mode
    if (process.env.USE_SHELL_PROMPT === 'true' && process.env.SHELL_PROMPT) {
        // Use the shell's PS1 prompt
        promptString = process.env.SHELL_PROMPT;
    }
    else if (config.prompt_style) {
        // Use custom prompt from config
        promptString = config.prompt_style;
    }
    else {
        // Use default prompt
        promptString = `${BLUE}$> ${RESET}`;
    }
...
- Reference earlier work when explaining current actions`;
    ...
    rl._writeToOutput = function (stringToWrite) {
        ...
        rl.output.write(BLUE + stringToWrite + RESET);
    };
    /**
     ...
     */
    function shouldContinueTask(response, toolUsed) {
        if (!currentTaskContext || currentTaskContext.isComplete)
            return false;
     ...
     */
    async function processUserInput(prompt, isAutoContinuation = false) {
        ...
        try {
            ...
            if (typeof result.text === "string" && result.text.trim() !== "") {
                ...
                toolResultObj = result.toolResults.find((tr) => typeof tr === "object" && tr !== null && tr.type === "tool-result");
                if (toolResultObj) {
            ...
            if (toolResultObj && toolName) {
                ...
                const toolSpinnerInterval = setInterval(() => {
                    process.stdout.write(`\r${YELLOW}Tool [${toolName}] is working... ${toolSpinnerFrames[toolSpinnerIndex]}${RESET}`);
